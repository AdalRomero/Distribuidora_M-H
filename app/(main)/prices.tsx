...
import { Calculator, Edit2, Loader2, Plus, Search, Trash2, Users, LayoutList } from 'lucide-react';
import { useState, useMemo, useEffect, useCallback, useRef } from 'react';
import { usePagination } from '../../src/hooks/usePagination';
import AddPrices, { TemplateFormData, TemplateRule } from '../../components/ui/modals/AddPrices';
...
export default function Prices() {
    ...
    const tablesToWatch = useMemo(() => ["precios_especiales_clientes", "precios_especiales_familias_clientes", "clientes"], []);
    ...
    // ==========================================
    // CARGAR LISTAS (DESDE PLANTILLAS)
    // ==========================================
    ...
    const loadPriceLists = useCallback(async () => {
        ...
        debounceRef.current = setTimeout(async () => {
            ...
            try {
                ...
                const [allPlantillas, allReglas, allClientes, allProductos, allFamilias] = await Promise.all([
                ...
                allFamilias.forEach((f: any) => familiaMap.set(f.id, `${f.codigoFamilia || 'S/C'} — ${f.nombre || 'Sin nombre'}`));
                ...
                const summaries: PriceListSummary[] = allPlantillas.map((p: any) => {
                    ...
                    const mappedReglas: TemplateRule[] = plantReglas.map((r: any) => ({
                        ...
                        targetId: r.targetId,
                        targetName: r.tipo === 'producto' ? productoMap.get(r.targetId) : familiaMap.get(r.targetId),
    ...
    useEffect(() => {
        const fire = () => { loadPriceListsRef.current?.(); };
    ...
    const handleSaveList = async (formData: TemplateFormData) => {
        ...
        try {
            ...
            await database.write(async () => {
                ...
                let plantilla: any;
                const existing = await plantillasDb.query(Q.where('nombre', formData.nombreLista)).fetch();
                if (existing.length > 0) {
                    plantilla = existing[0];
                    await plantilla.update((r: any) => { r.updated_at = Date.now(); });
                } else {
                ...
                const clientsToClear = allClients.filter((c: any) => c.lista_precio_base === formData.nombreLista && !formData.clientesIds.includes(c.id));
                for (const c of clientsToClear) {
                    await (c as any).update((r: any) => { r.lista_precio_base = null; });
                ...
                for (const cid of formData.clientesIds) {
                    ...
                    await c.update((r: any) => { r.lista_precio_base = formData.nombreLista; });
                    ...
                    for (const r of formData.reglas) {
                        if (r.tipo === 'producto') {
                            await pProdDb.create((rec: any) => {
                                rec.cliente_id = cid;
                                rec.producto_id = r.targetId;
                                rec.descuento_porcentaje = Number(r.descuentoPorcentaje) || 0;
                                rec.precio_fijo = Number(r.precioFijo) || 0;
                            });
                        } else {
                            await pFamDb.create((rec: any) => {
                                rec.cliente_id = cid;
                                rec.familia_id = r.targetId;
                                rec.descuento_porcentaje = Number(r.descuentoPorcentaje) || 0;
                            });
                        }
                    }
            ...
            setMessage({ type: 'success', text: 'Lista de precios guardada correctamente.' });
            ...
            setMessage({ type: 'error', text: 'Error al guardar lista: ' + error.message });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDeleteList = (item: PriceListSummary) => {
        if (item.nombreLista === 'Mayoreo' || item.nombreLista === 'Menudeo') {
            setMessage({ type: 'error', text: 'Las listas base (Mayoreo, Menudeo) no pueden ser eliminadas, solo editadas.' });
            return;
        }
        ...
        setWarningModalConfig({
            ...
            onConfirm: async () => {
                ...
                try {
                    const clientesDb = database.collections.get('clientes');
                    const preciosProdDb = database.collections.get('precios_especiales_clientes');
                    ...
                    await database.write(async () => {
                        for (const cid of item.clientesIds) {
                            ...
                            const oldProdRules = await preciosProdDb.query(Q.where('cliente_id', cid)).fetch();
                            for (const rule of oldProdRules as any[]) await rule.markAsDeleted();
                            
                            const oldFamRules = await preciosFamDb.query(Q.where('cliente_id', cid)).fetch();
                            for (const rule of oldFamRules as any[]) await rule.markAsDeleted();
                        }
                    });

                    setMessage({ type: 'success', text: 'Lista eliminada correctamente.' });
    ...
    const { visible: visibleLists, hasMore: hasMoreLists, loadMore: loadMoreLists, reset: resetPricePage } = usePagination(filteredLists, 5);
    useEffect(() => { resetPricePage(); }, [searchTerm]);
        ...
        <div className="p-4 sm:p-8 bg-slate-50 dark:bg-slate-900 min-h-screen font-sans">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-mh-blue-dark dark:text-white tracking-tight">Listas de Precios Especiales</h1>
                        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Gestiona plantillas de descuentos y asígnalas en masa a tus clientes.</p>
                    </div>
                    <button onClick={() => { setEditData(null); setRecoverData(null); setIsPriceModalOpen(true); }} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors font-medium text-sm shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center text-mh-blue dark:text-blue-400"><LayoutList className="w-6 h-6" /></div>
                        <div><p className="text-sm font-medium text-slate-500 dark:text-slate-400">Total de Listas</p><p className="text-2xl font-bold text-mh-blue-dark dark:text-white">{priceLists.length}</p></div>
                    </div>
                    <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center gap-4">
                        ...
                        <div><p className="text-sm font-medium text-slate-500 dark:text-slate-400">Clientes Asignados</p><p className="text-2xl font-bold text-mh-blue-dark dark:text-white">{priceLists.reduce((acc, l) => acc + l.clientesIds.length, 0)}</p></div>
                    </div>
                    <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-mh-pink/10 dark:bg-rose-900/30 flex items-center justify-center text-mh-pink dark:text-rose-400"><Calculator className="w-6 h-6" /></div>
                        <div><p className="text-sm font-medium text-slate-500 dark:text-slate-400">Reglas Activas</p><p className="text-2xl font-bold text-mh-blue-dark dark:text-white">{priceLists.reduce((acc, l) => acc + l.reglasCount, 0)}</p></div>
                    </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row gap-4 mb-6 bg-white dark:bg-slate-800 p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 items-center">
                    <div className="relative flex-1 w-full">
                        ...
                        <input type="text" className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-900 border-none rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none placeholder:text-slate-400" placeholder="Buscar por nombre de lista..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                </div>
                ...
                {isLoadingTable ? (
                    ...
                    <div className="py-12 text-center text-slate-400 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700">
                        {searchTerm ? 'No se encontraron listas que coincidan con la búsqueda.' : 'No hay listas de precios registradas. ¡Crea una nueva!'}
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {visibleLists.map((list) => (
                            <div key={list.nombreLista} className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col transition-all hover:shadow-md">
                                <div className="flex justify-between items-start mb-4">
                                    <h3 className="text-lg font-bold text-slate-800 dark:text-white truncate pr-4">{list.nombreLista}</h3>
                                    <div className="flex gap-1 shrink-0">
                                        <button onClick={() => startEdit(list)} className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/30 rounded-lg transition-colors" title="Editar Lista">
                                            <Edit2 className="w-4 h-4" />
                                        </button>
                                        <button onClick={() => handleDeleteList(list)} className="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-lg transition-colors" title="Eliminar Lista">
                                ...
                                <div className="flex flex-col gap-3 flex-1">
                                    <div className="flex items-center justify-between text-sm">
                                        <span className="text-slate-500 dark:text-slate-400 flex items-center gap-2"><Users className="w-4 h-4" /> Clientes</span>
                                        <span className="font-bold text-slate-700 dark:text-slate-200">{list.clientesIds.length}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-sm">
                                        <span className="text-slate-500 dark:text-slate-400 flex items-center gap-2"><Calculator className="w-4 h-4" /> Reglas de Excepción</span>
                                        <span className="font-bold text-slate-700 dark:text-slate-200">{list.reglasCount}</span>
                                    </div>
                                </div>

                                <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-700">
                                    <button onClick={() => startEdit(list)} className="w-full py-2 bg-slate-50 hover:bg-slate-100 dark:bg-slate-900/50 dark:hover:bg-slate-800 text-blue-600 dark:text-blue-400 font-bold rounded-lg transition-colors text-sm">
                                        Ver Configuración
                                    </button>
                                </div>
                            </div>
                ...
                {hasMoreLists && (
                    <div className="flex justify-center mt-6">
                        ...
                        >
                            Cargar más ({filteredLists.length - visibleLists.length} restantes)
                        </button>
                    </div>
                )}
