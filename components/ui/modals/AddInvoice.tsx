...
// ==========================================
function SearchableDropdown<T extends { id: string }>({ items, value, onChange, renderItem, renderSelected, placeholder, searchPlaceholder, disabled }: {
    items: T[]; value: string; onChange: (item: T) => void;
    renderItem: (item: T) => React.ReactNode; renderSelected: (item: T) => string;
    ...
    return (
        <div ref={ref} className="relative">
            <button type="button" disabled={disabled} onClick={() => setIsOpen(!isOpen)}
                ...
                <span className={selected ? 'text-slate-700 dark:text-slate-200' : 'text-slate-400'}>{selected ? renderSelected(selected) : placeholder}</span>
                ...
                <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl max-h-56 flex flex-col overflow-hidden">
                    <div className="p-2 border-b border-slate-100 dark:border-slate-700">
                        <div className="relative">
                            ...
                            <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder={searchPlaceholder}
                                className="w-full pl-8 pr-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-900 border-none rounded-lg outline-none text-slate-700 dark:text-slate-300 placeholder:text-slate-400" autoFocus />
                        </div>
                    </div>
                    <div className="overflow-y-auto flex-1">
                        {filtered.length === 0 ? (
                            ...
                            filtered.map(item => (
                                <button key={item.id} type="button" onClick={() => { onChange(item); setIsOpen(false); setSearch(''); }}
...
export default function AddInvoice({ isOpen, onClose, recoverData, onSaveSuccess }: AddInvoiceProps) {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            setClientes(allC.map((c: any) => ({
                ...
                listaPrecioBase: c.listaPrecioBase || 'lista', descuentoGlobal: c.descuentoGlobal || 0,
                contacto: c.contacto || '', calle: c.calle || '', colonia: c.colonia || '', cp: c.cp || '', ciudad: c.ciudad || '', estado: c.estado,
            })));

            setProductos(allP.filter((p: any) => p.estado).map((p: any) => ({
    ...
    useEffect(() => {
        if (recoverData) {
            setForm({
                serie: recoverData.serie || 'A', folio: recoverData.folio || '1',
                fecha: recoverData.fecha ? new Date(recoverData.fecha).toISOString().split('T')[0] : initialForm.fecha,
                hora: initialForm.hora, tipoComprobante: recoverData.tipoComprobante || 'I',
                lugarExpedicion: recoverData.lugarExpedicion || '83554', metodoPago: recoverData.metodoPago || 'PPD',
                formaPago: recoverData.formaPago || '99', moneda: recoverData.moneda || 'MXN',
                codigoCliente: recoverData.codigoCliente || '', nombre: recoverData.nombre || '',
                rfc: recoverData.rfc || 'XAXX010101000', domicilio: recoverData.domicilio || '',
                agente: recoverData.agente || '', usoCFDI: recoverData.usoCFDI || 'S01',
                observaciones: recoverData.observaciones || '', tipoRelacion: recoverData.tipoRelacion || '', cfdiRelacionado: recoverData.cfdiRelacionado || '', clienteId: recoverData.clienteId || '',
                conceptos: recoverData.conceptos?.length ? recoverData.conceptos.map((c: any) => ({
                    id: Math.random().toString(36).slice(2), cantidad: c.cantidad ? String(c.cantidad) : '',
                    unidadSat: c.unidadSat || 'H87', claveSat: c.claveSat || '', concepto: c.concepto || '',
                    valorUnitario: c.valorUnitario ? String(c.valorUnitario) : '', descuento: c.descuento ? String(c.descuento) : '',
                    porcImpuesto: c.porcImpuesto ? String(c.porcImpuesto) : '16', productoId: c.productoId || '',
                })) : [newConcepto()]
            });
        }
    }, [recoverData]);
    ...
    const handleSelectCliente = (cliente: ClienteItem) => {
        const direccion = [cliente.calle, cliente.colonia, cliente.cp, cliente.ciudad].filter(Boolean).join(', ');
        setForm(prev => ({
            ...prev,
    ...
    const handleSelectProducto = (conceptoId: string, producto: ProductoItem) => {
        ...
        const precioEsp = preciosEspeciales.find(pe => pe.clienteId === form.clienteId && pe.productoId === producto.id);
        ...
        if (selectedCliente) {
            ...
            switch (selectedCliente.listaPrecioBase) {
                case 'mayoreo': precio = producto.precioMayoreo || producto.precioLista; break;
                case 'menudeo': precio = producto.precioMenudeo || producto.precioLista; break;
    ...
    const getPriceBadge = (conceptoProductoId: string) => {
        ...
        const pe = preciosEspeciales.find(p => p.clienteId === form.clienteId && p.productoId === conceptoProductoId);
        const cliente = clientes.find(c => c.id === form.clienteId);

        if (pe && pe.precioFijo > 0) return <span className="ml-1 px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 rounded text-[9px] font-bold">Precio Especial</span>;
        if (pe && pe.descuentoPorcentaje > 0) return <span className="ml-1 px-1.5 py-0.5 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 rounded text-[9px] font-bold">Desc. Especial {pe.descuentoPorcentaje}%</span>;
        if (cliente && cliente.descuentoGlobal > 0) return <span className="ml-1 px-1.5 py-0.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 rounded text-[9px] font-bold">Desc. Global {cliente.descuentoGlobal}%</span>;
        if (cliente) {
            const lbl = cliente.listaPrecioBase === 'mayoreo' ? 'Mayoreo' : cliente.listaPrecioBase === 'menudeo' ? 'Menudeo' : 'Lista';
            return <span className="ml-1 px-1.5 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 rounded text-[9px] font-bold">{lbl}</span>;
        }
        return null;
    };
                ...
                <div className="w-[95vw] max-w-7xl h-[90vh] bg-white dark:bg-slate-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden relative z-10">
                    ...
                    <div className="flex items-center justify-between px-6 py-3 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shrink-0">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-blue-100 dark:bg-blue-900/30 rounded-lg"><FileText className="w-5 h-5 text-blue-700 dark:text-blue-400" /></div>
                            <div>
