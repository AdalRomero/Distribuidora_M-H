...
// Images served from public/ directory (Expo web serves these as static files)
...
export default function Login() {
    ...
    const whiteBorder = '[text-shadow:_-1px_-1px_0_#fff,_1px_-1px_0_#fff,_-1px_1px_0_#fff,_1px_1px_0_#fff,_0_3px_6px_rgba(0,0,0,0.15)]';
    ...
    return (
        <div className={`flex min-h-screen w-full transition-all duration-700 ease-in-out ${isFormLeft ? 'flex-row' : 'flex-row-reverse'}`}>

            <div className={`relative z-10 w-full lg:w-[400px] xl:w-[450px] shrink-0 flex flex-col justify-center px-8 sm:px-12 py-8 shadow-2xl transition-colors duration-500 ${formThemeClass}`}>
                ...
                <div className="text-center mb-8">
                    ...
                    <h2 className={`text-2xl font-bold mt-1 transition-all duration-300 ${headingClass}`}>
                        Iniciar Sesión
                    </h2>
                    <p className={`mt-2 text-sm font-medium transition-colors duration-300 ${isCaja ? 'text-gray-800' : 'text-pink-900'}`}>
                ...
                <div className="flex justify-center gap-10 mb-8 relative">
                    <button
                        ...
                        className={`relative flex items-center justify-center w-16 h-16 rounded-full border-2 transition-all duration-500 ease-out overflow-visible ${isCaja
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <label htmlFor="username" className={`block text-sm font-semibold mb-1 pointer-events-none transition-colors duration-300 ${labelClass}`}>
                            Usuario
                        </label>
                    ...
                    <div>
                        <label htmlFor="accessNumber" className={`block text-sm font-semibold mb-1 pointer-events-none transition-colors duration-300 ${labelClass}`}>
                            Contraseña
                        </label>
                        <div className="relative">
                            <button
                            ...
                            >
                                {showPassword ? (
                                    <Unlock className={`w-5 h-5 transition-colors duration-300 ${iconColorClass}`} />
                                ) : (
                                    <Lock className={`w-5 h-5 transition-colors duration-300 ${iconColorClass}`} />
                                )}
                            </button>
                            <input
                                id="accessNumber"
