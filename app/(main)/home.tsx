...
export default function Home() {
    const renderBadge = (status: string) => {
        switch (status) {
            ...
            default:
                return <span className="px-3 py-1 text-xs rounded-full bg-slate-100 text-slate-700 font-medium">{status}</span>;
        }
    };
        ...
        <div className="p-8 bg-slate-50 min-h-screen font-sans">
            <div className="max-w-7xl mx-auto">
                <header className="mb-8">
                    <h1 className="text-4xl font-extrabold text-slate-800 tracking-tight">Panel de Información</h1>
                    <p className="text-slate-500 mt-2 text-sm">Resumen general de contratos y operaciones métricas</p>
                </header>
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-8">
                            ...
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col">
                                ...
                                <div className="flex-1 min-h-[16rem] flex flex-col items-center justify-center relative">
                                    ...
                                    <div className="absolute flex flex-col items-center justify-center pointer-events-none">
                                        <span className="text-4xl font-extrabold text-slate-800">830</span>
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-lg font-bold text-slate-800">Mis Contratos</h3>
                                <button className="text-sm text-[#6383a1] font-semibold hover:text-[#85a3bf] transition-colors">Ver todos →</button>
                            </div>
                            <div className="overflow-x-auto">
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b-2 border-slate-100 text-[11px] text-slate-400 uppercase tracking-widest">
                                    ...
                                    <tbody className="text-sm">
                                        {contractsData.map((contract, i) => (
                                            <tr key={i} className="border-b border-slate-50 hover:bg-slate-50/70 transition-colors last:border-0 group">
                                                <td className="py-4 px-2 text-slate-500 font-medium group-hover:text-[#6383a1] transition-colors">{contract.serial}</td>
                                                <td className="py-4 px-2 text-slate-800 font-medium">{contract.name}</td>
                                                <td className="py-4 px-2 text-right font-bold text-slate-700">{contract.value}</td>
                    ...
                    <div className="lg:col-span-1 flex flex-col gap-20">
                                ...
                                {progressBars.map((bar, i) => (
                                    <div key={i}>
                                        <div className="flex justify-between text-sm mb-2">
                                            ...
                                            <span className="text-slate-500 font-bold">{bar.percent}%</span>
                                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-7">
                            {timeCards.map((card, i) => (
                                <div key={i} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 text-center flex flex-col justify-center items-center group hover:-translate-y-1 transition-transform duration-300">
                                    <span className="text-4xl font-extrabold text-slate-800 group-hover:text-[#6383a1] transition-colors">
                                        {card.days}
                                    </span>
                                    <span className="text-[11px] text-slate-500 font-bold uppercase tracking-wider mt-3 leading-tight opacity-80">
                                        {card.label}
                                    </span>
                                </div>
                            ))}
