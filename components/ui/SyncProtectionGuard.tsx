...
export default function SyncProtectionGuard() {
  ...
  const handleRecoverFresh = async () => {
    if (!confirm("Esta acción borrará los registros locales del dispositivo y los volverá a descargar todos limpios desde Supabase. Es completamente seguro y no afectará los datos en la nube. ¿Deseas continuar?")) {
      return;
    }
    setLoading(true);
    setErrorMsg(null);
    try {
      ...
      alert("Base de datos local restablecida con éxito. Iniciando sincronización fresca...");
  ...
  const handleBypassSync = async () => {
    ...
    try {
      ...
      setBlockedReason(null);
      alert("Sincronización forzada con éxito. Los borrados masivos han sido aplicados.");
    } catch (e: any) {
      console.error(e);
