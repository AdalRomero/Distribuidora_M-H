...
function ProductLotsViewInner({ producto, lotes, onBack, onAddLote, onEditLote, onToggleEstado }: Props) {
    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 animate-in slide-in-from-right-4 duration-300">
            <div className="flex items-center justify-between mb-6 border-b border-slate-100 pb-4">
                ...
                <button onClick={onAddLote} className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-all shadow-sm shadow-blue-200 font-medium text-sm">
                    <Plus className="w-4 h-4" /><span>Registrar Lote</span>
                </button>
            </div>
                ...
                <table className="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {lotes.length === 0 ? (
                            ...
                            return (
                                <tr key={lote.id} className={`hover:bg-slate-50/50 transition-colors group ${!lote.estado ? 'border-l-2 border-l-slate-300 opacity-60 bg-slate-50/30' : isExpired ? 'border-l-2 border-l-rose-500 bg-rose-50/10' : 'border-l-2 border-l-emerald-500'}`}>
